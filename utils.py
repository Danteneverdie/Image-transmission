...
def get_subbands_info(img_size=512, num_levels=5):
    ...
    for lvl in range(num_levels, 0, -1):
        sz = img_size // (2 ** lvl)
        # HL (右上), LH (左下), HH (右下)
        # lvl代表层级，r0, r1 代表行范围，c0, c1 代表列范围
...
# 计算 DWT 矩阵对应的初始阈值 T0，小于最大值的2的幂次方
...
def has_significant_descendant(r, c, sub, T, coeffs, subbands):
    ...
    child_sub = None
    for s in subbands:
        if s['lvl'] == lvl - 1 and s['type'] == sub['type']:
            child_sub = s
            break

    # 下一层子代对应的 2x2 区域左上角坐标
...
def mark_subtree_skip(r, c, sub, skip_curr, subbands):
    ...
    while stack:
        ...
        if skip_curr[curr_r, curr_c]:
            continue
        # 在本轮跳过图（skip_curr）上，将当前后代标记为 True（跳过）
        ...
        if curr_lvl > 1:
            child_sub_next = None
            for s in subbands:
                if s['lvl'] == curr_lvl - 1 and s['type'] == curr_sub['type']:
                    child_sub_next = s
                    break
...
def ezw_scan_2d(coeffs, initial_threshold, img_size=512, num_levels=5):
    ...
    # 追踪重建幅度的矩阵，用来辅扫描
    recon_mag = np.zeros((img_size, img_size), dtype=np.float64)
    # 辅扫描列表，记录显著系数的坐标
    subordinate_list = []
    ...
    while T >= 1:
        ...
        # 主扫描
        for sub in subbands:
            for r in range(sub['r0'], sub['r1']):
                for c in range(sub['c0'], sub['c1']):
                    if sig[r, c]:
                        continue
                    if skip_curr[r, c]:
                        continue
                    ...
                    if abs(val) >= T:
                        ...
                        recon_mag[r, c] = 1.5 * T
                        # 将坐标加入辅扫描列表
                        subordinate_list.append((r, c))
                    else:
                        # 判定为不显著：判断是否为零树根
                        if has_significant_descendant(r, c, sub, T, coeffs, subbands):
                            symbols.append('I')  # 隔离零
                        else:
                            symbols.append('Z')  # 零树根
                            mark_subtree_skip(r, c, sub, skip_curr, subbands)

        # 辅助扫描
        delta = T / 4.0  # 本轮逼近的调整步长
        for r, c in subordinate_list:
            val_mag = abs(int(coeffs[r, c]))
            # 如果真实幅度>当前的重建幅度
            if val_mag >= recon_mag[r, c]:
                symbols.append('1')  # 记为比特 '1'
                recon_mag[r, c] += delta  # 重建幅度上调
            else:
                symbols.append('0')  # 记为比特 '0'
                recon_mag[r, c] -= delta  # 重建幅度下调
...
def ezw_inverse_scan_2d(symbols, initial_threshold, img_size=512, num_levels=5):
    ...
    while T >= 1:
        ...
        for sb in subbands:
            for r in range(sb['r0'], sb['r1']):
                for c in range(sb['c0'], sb['c1']):
                    ...
                    if symbol == 'P':
                        recon_coeffs[r, c] = 1.5 * T  # 初始重构为 1.5T
                        sig[r, c] = True
                        subordinate_list.append((r, c))
                    elif symbol == 'N':
                        recon_coeffs[r, c] = -1.5 * T # 初始重构为 -1.5T
        ...
        for r, c in subordinate_list:
            ...
            if symbol == '1':
                # 细化为上限区间，绝对值变大
                if recon_coeffs[r, c] >= 0:
                    recon_coeffs[r, c] += delta
                else:
                    recon_coeffs[r, c] -= delta
            elif symbol == '0':
                # 细化为下限区间，绝对值变小
